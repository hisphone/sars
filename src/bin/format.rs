...
fn main() -> anyhow::Result<()> {
    ...
    println!("开始读取{:?}", path.join("11/收入.txt"));
    let incoming = Incomings::from_txt(path.join("11/收入.txt"))?;

    println!("开始写入{:?}", path.join("formatt/入账信息表.xlsx"));
    incoming.to_excel(&path.join("formatt/入账信息表.xlsx").to_str().unwrap())?;

    println!("开始读取{:?}", path.join("11/支出.xls"));
    let outgoing = Outgoings::from_excel(&path.join("11/支出.xls"))?;

    println!("开始写入{:?}", path.join("formatt/支出信息表.xlsx"));
    outgoing.to_excel(&path.join("formatt/支出信息表.xlsx").to_str().unwrap())?;

    println!("开始读取{:?}", path.join("11/退款.xls"));
    let refunds = Refunds::from_excel(&path.join("11/退款.xls"))?;

    println!("开始写入{:?}", path.join("formatt/退款信息表.xlsx"));
    refunds.to_excel(&path.join("formatt/退款信息表.xlsx").to_str().unwrap())?;

    println!("开始读取{:?}", path.join("11/余额.txt"));
    let balance = Balances::from_txt(&path.join("11/余额.txt"))?;
